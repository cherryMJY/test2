...
class Neo4j(object):
    ...
    def quesNodeDictUseIdAndLabel(self, label, neo4jId):
        """
        通过类目名和id查询
        :param label:
        :param neo4jId:
        ...
        sql = sql + ") "
        if (neo4jId != None):
            sql = sql + " WHERE id(n) = " + str(neo4jId)
        sql = sql + " return n"
        res = self.graph.run(sql)
        retDict = {}
        for a in res:
    ...
    def deleteNode(self, label, attributeDict):
        """
        删除节点
        :param label:
        :param attributeDict:
        ...
        if (attributeDict != None):
            sql = sql + "where "
            cnt = 0
            for val in attributeDict.keys():
                if (cnt != 0):
                    sql = sql + ","
                if (val == 'id'):
                    sql = sql + "id(n)" + "=" + str(attributeDict[val]) + " "
                else:
                    sql = sql + "n." + val + "=" + str(attributeDict[val]) + " "
                cnt += 1
        sql = sql + " DETACH DELETE n"
        # print(sql)
        self.graph.run(sql)
        # print('成功')
    ...
    def match(self, object_from=None, relationship=None, object_to=None, return_info="1"):
        """
        搜索neo4j中所有符合条件的数据
        :param object_from:主节点搜索内容，格式为{"label_name":"类目名", "content": {搜索关键字段}}，下同
        :param relationship:
        :param object_to:
        :param return_info:返回的信息数据，123分别对应object_from，relationship和object_to
        :return:
        """
        # a = Node('label', name='a')
        # self.graph.create(a)
        ...
        if object_from is not None:
            ...
            if "content" in object_from.keys() and "id" in object_from["content"]:
                search_id_list.append("id(n1) = " + str(object_from["content"]["id"]))
            elif "content" in object_from.keys():
                sql += self.json2str(object_from["content"])
            sql += ")"
        if object_to is not None:
            sql += ",(n3:" + object_to["label_name"]
            if "content" in object_to.keys() and "id" in object_to["content"]:
                search_id_list.append("id(n3) = " + str(object_to["content"]["id"]))
            elif "content" in object_to.keys():
        ...
        if len(search_id_list) > 0:
            sql += " where " + search_id_list[0]
            for i in range(1, len(search_id_list)):
                sql += " and " + search_id_list[i]
    ...
    def ret_node_list_get_one_category_node_name_id(self, node_type, attribute_name, category_id):
        ...
        """
        sql = "match(n:" + node_type + ") return id(n),n."+attribute_name + " LIMIT 25"
        node_list = []
        res = self.graph.run(sql)
        num = 1
    ...
    def update_one_node_label(self, label_name, ques_attribute_name, ques_attribute_context, update_lable_name):
        ...
        根据id 修改标签名字 输入是 标签名 修改的属性和属性内容  新的标签名字
        :param label_name:
        :param ques_attribute_name:
        :param ques_attribute_context:
        :param update_lable_name:
        :return:
        """
    ...
    def update_one_node(self, label_name, query_info, update_info):
        """
        更新一个结点数据
        :param label_name:类目名
        :param query_info:查询属性，json格式，用该信息查询要修改的结点
        :param update_info:更新属性，json格式，目标结点要更新的属性
        :return:
        """
        try:
            sql = "MATCH(a: " + str(label_name) + ")"
            query_list = []
            for key in query_info.keys():
                if key == "id":
                    query_list.append("id(a)=" + str(query_info[key]))
                else:
                    query_list.append("a." + key + " = " + str(query_info[key]))
            if len(query_list) >= 1:
                sql += " WHERE " + query_list[0]
                for i in range(1, len(query_list)):
                    sql += " and " + query_list[i]
            sql += " SET "
            update_list = []
            for key in update_info.keys():
                if key != "id":
                    update_list.append("a." + key + " =" + str(update_info[key]))
    ...
    def update_attribute_key(self, category_id, map_rule_id, category_name, include_name, change_to_name,
                         map_attribute_id, old_name_show_in_database, create_id, repo_id, object_from=None,
        ...
        :param category_name:
        :param include_name:
        :param change_to_name:
        :param map_attribute_id:
        :param old_name_show_in_database:
        :param create_id:
        :param repo_id:
        :param object_from:
        :param relationship:
        ...
        for val in ret:
            ...
            if (change_to_name == include_name):
                update_sql = "match(n:" + str(category_name) + ") where n._id=\'" + str(_id) + "\' set n." + str(change_to_name) + "=n." + str(include_name)
            else:
                update_sql = "match(n:" + str(category_name) + ") where n._id=\'" + str(_id) + "\' set n." + str(change_to_name) + "=n." + str(include_name) + " remove n." + str(include_name)
            #print(update_sql)
            ...
            _repo_id = repo_id
            # 删除上面的关于这条规则的日志
            return_map_attribute_log = TAttributeMapLog.objects.filter(map_rule_id=map_rule_id, create_id=create_id,
                                                                       repo_id=repo_id)
        ...
        if (map_attribute_id == -1):
            return_att_map_log = TAttributeMapLog.objects.filter(map_rule_id=map_rule_id, category_id=category_id,
                                                                 create_id=create_id, repo_id=repo_id)
